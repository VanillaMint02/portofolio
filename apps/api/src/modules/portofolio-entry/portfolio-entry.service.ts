...
export class PortfolioEntryService {
    ...
    private async readOneById(portfolioEntryId: string): Promise<PortfolioEntryDomain> {
        const foundModel = await this.portfolioEntryRepository.findOneBy({id: portfolioEntryId});
        if (!foundModel) {
    ...
    async getAllPortfolioEntries():Promise<PortfolioEntryDto[]>{
        const foundModels=await this.portfolioEntryRepository.find({relations:['user']});
        if(!foundModels){
            return [];
        }
        return foundModels.map(model=>PortfolioEntryMapper.mapToDto(model));
    }
    async createPortfolioEntry(createPortfolioEntryDto: CreatePortfolioEntryDto): Promise<PortfolioEntryDto> {
        const foundUserModel = await this.userService.getUserById(createPortfolioEntryDto.ownerId);
        ...
        try {
            const portfolioEntryDomain = PortfolioEntryMapper.mapCreatePortfolioEntryToDomain(createPortfolioEntryDto);
            portfolioEntryDomain.user = UserMapper.mapToDomain(foundUserModel);
            const savedPortfolioEntryDomain = await this.portfolioEntryRepository.save(portfolioEntryDomain);
    ...
    async updatePortfolioEntry(portfolioEntryDto: PortfolioEntryDto): Promise<PortfolioEntryDto> {
        await this.findOneById(portfolioEntryDto.id);
        const savedPortfolioEntry = PortfolioEntryMapper.mapToDomain(portfolioEntryDto);
